...
OUTPUT_PATH="/home/dagivi/articulo_diego/resultados/IFG_IGT"
...
python $SPARCC_PATH/SparCC.py $INPUT_PATH/tabla_genera_IFG_IGT.txt -i $ITER --cor_file=$OUTPUT_PATH/tabla_genera_sparcc_IFG_IGT.txt > $OUTPUT_PATH/sparcc_IFG_IGT.log
...
echo "$INPUT_PATH"
cp -v $INPUT_PATH/tabla_genera_IFG_IGT.txt .

echo "now let's make a ls to verify that the files were created"
list=$(ls)
echo "$list"

python $SPARCC_PATH/MakeBootstraps.py tabla_genera_IFG_IGT.txt -n $BOOT_ITER --path=$OUTPUT_PATH/Resamplings/boot
...
do
    python $SPARCC_PATH/SparCC.py $OUTPUT_PATH/Resamplings/boottabla_genera_IFG_IGT.txt.permuted_$i.txt -i $ITER --cor_file=$OUTPUT_PATH/Bootstraps/sim_cor_$i.txt >> $OUTPUT_PATH/sparcc_IFG_IGT.log
done

# compute p-value from bootstraps
python $SPARCC_PATH/PseudoPvals.py $OUTPUT_PATH/tabla_genera_sparcc_IFG_IGT.txt $OUTPUT_PATH/Bootstraps/sim_cor_"#".txt $ITER -o $OUTPUT_PATH/pvals_one_sided_IFG_IGT.txt -t 'one_sided'  >> $OUTPUT_PATH/sparcc_IFG_IGT.log
